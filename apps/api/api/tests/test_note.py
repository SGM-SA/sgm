...
class NoteViewsTestCase(TestCase):
    ...
    def test_retrieve_note(self):
        note = Note.objects.create(contenu="Test note for retrieve", affaire=self.affaire, user=self.user)
    ...
    def test_update_note(self):
        note = Note.objects.create(contenu="Test note for update", affaire=self.affaire, user=self.user)
        url = f"/api/notes/{note.id}"
    ...
    def test_delete_note(self):
        note = Note.objects.create(contenu="Test note for delete", affaire=self.affaire, user=self.user)
        url = f"/api/notes/{note.id}"
