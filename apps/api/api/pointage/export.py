...
def export_pointages(start_date, end_date):
    ...
    """
    pointages = PointageEtape.objects.exclude(
      date_fin=None
    ).filter(
    ...
    response['Content-Disposition'] = f'attachment; filename="pointages_{start_date}_{end_date}.xlsx"'
    wb.save(response)
