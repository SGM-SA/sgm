...
class PointageGestion(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not etape_id:
          return Response({"detail": "Étape ID est nécessaire"}, status=status.HTTP_400_BAD_REQUEST)

        try:
          etape = Etape.objects.get(id=etape_id)
        except Etape.DoesNotExist:
          return Response({"detail": "Étape n'existe pas"}, status=status.HTTP_404_NOT_FOUND)

        current_pointage = PointageEtape.objects.filter(user=user, date_fin__isnull=True).first()
        ...
        if current_pointage:
          if current_pointage.etape == etape:
            ...
            return Response({"detail": "Pointage arrêté"}, status=status.HTTP_200_OK)
          else:
            # Stop le pointage en cours et crée un nouveau pointage si l'étape est différente
            current_pointage.date_fin = timezone.now()
            current_pointage.save()
            new_pointage = PointageEtape(user=user, etape=etape, date_debut=timezone.now())
            new_pointage.save()
            return Response({"detail": "Ancien Pointage stoppé, nouveau créé"}, status=status.HTTP_200_OK)
        else:
          new_pointage = PointageEtape(user=user, etape=etape, date_debut=timezone.now())
          new_pointage.save()
